...
from APPS.ORDER.models import Cart, TempOrder
...
def product_search(request, product_name, page_num):
    ...
    if(rescode == 200):
        ...
        for data in items['items']:
            try:
                Product.objects.get(product = int(data['productId']))
...
def product_best(request):
    ...
    for data in items:
        try:
            Product.objects.get(product = int(data['nvMid']))
        except Product.DoesNotExist:
...
def before_pay(request, product_id):
    ...
    if not TempOrder.objects.filter(product_id=product_id).exists():
        TempOrder(product_id=product_id, member_id=member_id, quantity=quantity, price=int(price)).save()
        
    return render(request, 'before_pay.html', {'product_id' : product_id})
